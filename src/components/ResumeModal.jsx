...
import { X, Printer, Download, Mail, Phone, MapPin, Globe, ExternalLink, GraduationCap, Briefcase } from 'lucide-react';
...
export default function ResumeModal({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 overflow-y-auto bg-black/85 backdrop-blur-xl print:p-0 print:bg-white print:fixed">
      {/* Container */}
      <div className="relative w-full max-w-4xl bg-[#0d1017] border border-white/10 rounded-3xl shadow-2xl overflow-hidden my-6 print:border-none print:shadow-none print:m-0 print:bg-white print:text-black">
        
        {/* Floating Action Bar (hidden in print) */}
        <div className="p-4 bg-black/50 border-b border-white/5 flex items-center justify-between print:hidden">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
              className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold transition-colors shadow-lg"
            >
        ...
        <div className="p-6 sm:p-10 max-h-[80vh] overflow-y-auto print:max-h-none print:overflow-visible space-y-6 text-slate-200 print:text-gray-900">
          ...
          <div className="text-center border-b border-slate-800 print:border-gray-300 pb-6">
            ...
            <div className="flex flex-wrap justify-center items-center gap-3 text-xs text-cyan-400 print:text-blue-700 font-mono mt-1">
              <a href={personalInfo.linkedin} target="_blank" rel="noreferrer">LinkedIn: linkedin.com/in/{personalInfo.linkedinUsername}</a>
              <span>•</span>
              <a href={personalInfo.github} target="_blank" rel="noreferrer">GitHub: github.com/{personalInfo.githubUsername}</a>
            </div>
          </div>
              ...
              {experienceData.map((exp, idx) => (
                <div key={idx} className="text-xs sm:text-sm">
                  <div className="flex justify-between items-baseline font-bold text-white print:text-black">
                    <span>{exp.role} — <span className="font-normal text-cyan-400 print:text-blue-800">{exp.company}</span></span>
                    <span className="font-mono text-slate-400 print:text-gray-600 text-xs">{exp.period}</span>
                  </div>
          ...
          <div>
            ...
            <div className="space-y-1.5 text-xs text-slate-300 print:text-gray-800">
              <p><strong className="text-white print:text-black">Core Data Science:</strong> Machine Learning, Data Analysis, Visualization, Predictive Modeling, SQL/NoSQL, RAG, Prompt Engineering</p>
              <p><strong className="text-white print:text-black">Mobile Development:</strong> Flutter, Dart, Firebase, REST API Integration, Cross-platform Architecture</p>
              ...
              <p><strong className="text-white print:text-black">Languages:</strong> Python, Java, C, R, JavaScript, PHP, Dart</p>
            </div>
          </div>
