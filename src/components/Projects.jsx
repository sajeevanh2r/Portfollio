      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <h2 className="text-3xl sm:text-5xl font-extrabold text-white tracking-tight">
            Things I <span className="text-gradient-cyan">Built</span>
          </h2>
          <p className="mt-4 text-slate-300 text-base">
            Each project represents a problem I was fascinated by — from predictive AI pipelines and vector search to production mobile apps.
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between mb-10">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-medium transition-all ${
                  activeCategory === cat
                    ...
                    : 'cinematic-pill text-slate-300 hover:text-white hover:bg-slate-800'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-900/90 border border-slate-800 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500 transition-colors"
            />
          </div>
        </div>
        ...
        {filteredProjects.length === 0 ? (
          <div className="cinematic-card p-12 rounded-3xl text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProjects.map((project) => (
              <div
                key={project.id}
                className="cinematic-card rounded-3xl p-6 sm:p-7 flex flex-col justify-between group relative overflow-hidden h-full shadow-lg"
              >
                {/* Top Badge Strip */}
                <div>
                  <div className="flex items-center justify-between gap-2 mb-4">
                    <span className="px-2.5 py-1 rounded-full text-[11px] font-mono font-semibold bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
